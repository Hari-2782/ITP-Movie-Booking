  ...
  const updateMovieSeatLayout = async (req, res) => {
    try {
        ...
        for (let i = 0; i < numRows; i++) {
          ...
          const rowname = `${String.fromCharCode(65 + i)}`; // Generate rowname like "silver-A", "silver-B", etc.
        ...
        const updatedMovieSeatLayout = await screen.findByIdAndUpdate(
            req.params.id,
            { $push: { seats: { type, rows, price } } }, // Use $push operator to append to the seats array
            { new: true }
        );
